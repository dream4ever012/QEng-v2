...
def comparePerf_std (TableName_mid, TableName_left, TableName_right,
    ...
    for i in range(0,upper):
        err_sum_upper = err_sum_upper + (card_upper_act[i] - card_upper_est[i])**2
        #err_sum_upper = err_sum_upper + (card_upper_act[i] - card_upper_est[i])
    err_std_upper = (float(err_sum_upper)/(max(len(card_upper_act),1)))**0.5
    
    err_sum_under = 0
    for i in range(0,under):
        err_sum_under = err_sum_under + (card_under_act[i] - card_under_est[i])**2
        #err_sum_under = err_sum_under + (card_under_act[i] - card_under_est[i])
    err_std_under = (float(err_sum_under)/(max(len(card_under_act),1)))**0.5
